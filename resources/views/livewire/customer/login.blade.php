        ...
        <div class="card lg:max-w-lg w-full bg-base-100 shadow-xl" id="step-1">
            <div class="card-body">
                ...
                <p class="font-medium text-base-content text-opacity-65 text-sm">
                    {{ __('distributor_frontend.sign_in') }}</p>
                <form wire:submit="login">
                    ...
                    <input type="password" placeholder="{{ __('distributor_frontend.password') }}"
                        class="input input-bordered w-full mb-2" wire:model='password' />

                    <a href="{{ route('customer.password.request') }}"
                        class="font-medium text-primary text-sm">{{ __('distributor_frontend.forgot_or_lost_your_password') }}</a>

                    <div class="w-full flex flex-wrap justify-between mt-3">
                        <div class="card-actions justify-start">
                            <button type="submit"
                                class="btn btn-cine-highlight-1 text-opacity-100 font-bold">{{ __('distributor_frontend.login') }}</button>
                        </div>
                    </div>
                </form>
            </div>
