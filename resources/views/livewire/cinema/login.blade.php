    ...
    <div class="min-h-screen flex flex-col flex-wrap justify-center content-center gap-8" x-data="{
        ...
        <div class="card lg:max-w-lg w-full bg-base-100 shadow-xl" id="step-1">
            <div class="card-body">
                ...
                <p class="font-medium text-base-content text-opacity-65 text-sm">{{ __('cinema_frontend.sign_in') }}</p>
                <form wire:submit="login">
                    <input type="email" placeholder="{{ __('cinema_frontend.email_address') }}"
                        class="input input-bordered w-full mt-4 mb-2" wire:model='email' x-bind:disabled="isDisabled" />

                    <div class="w-full flex flex-wrap justify-between mt-3">
                        <div class="card-actions justify-start">
                            <button type="submit" class="btn btn-cine-highlight-1 text-opacity-100 font-bold"
                                x-bind:disabled="isDisabled">{{ __('cinema_frontend.login') }}</button>
                        </div>
                    </div>
                </form>
            </div>
