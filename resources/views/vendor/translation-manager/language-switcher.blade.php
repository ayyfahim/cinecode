...
<x-filament::dropdown placement="bottom-start">
    <x-slot name="trigger">
        <button @class(['block hover:opacity-75', 'pt-0' => $showFlags]) id="filament-language-switcher" @click.prevent="toggle">
            <div @class([
                ...
                <span class="opacity-100">
                    ...
                    @else
                        <x-icon name="heroicon-o-language" class="w-5 h-5" />
                    @endif
                </span>
            </div>
        </button>
        ...
        @foreach ($otherLanguages as $language)
            ...
            <x-filament::dropdown.list.item>
                <a @class([
                    ...
                    @if (!$isCurrent) href="{{ route('translation-manager.switch', ['code' => $language['code']]) }}" @endif>
                    <span
                        class="filament-dropdown-list-item-label truncate w-full text-start flex justify-content-start gap-3">
                        @if ($showFlags)
                            {{ try_svg('flag-4x3-' . $language['flag'], 'w-6 h-6') }}
                            <span class="pl">{{ $language['name'] }}</span>
                        @else
                            <span
                                @class(['font-semibold' => $isCurrent])>{{ str($language['code'])->upper()->value() . " - {$language['name']}" }}</span>
                        @endif
                    </span>
                </a>
            </x-filament::dropdown.list.item>
